import { getStockOverview, getWarehouses, getStockMovements } from '@/lib/supabase/stock'
      ...
      <header className="border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-md px-8 py-5 flex items-center justify-between sticky top-0 z-40">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400">Almoxarifado & Depósitos</span>
          </div>
          <h1 className="text-2xl font-black text-white mt-1">Estoque Multi-Depósito & KARDEX</h1>
        </div>
      </header>

      {/* Conteúdo */}
      <main className="p-8 max-w-7xl mx-auto w-full flex-1">
        <EstoqueView items={items} warehouses={warehouses} movements={movements} />
      </main>
    </div>
  )
}
